...
int ForceGetCh()
{
        ...
        if (!ReachedEOF)
        {
                ...
                while (CrtStatus() == (char)0)
                {
                        // read another record
                        if(!fgets(tmpbuf, LINE_SIZE_MAX + 1, fp))
                        {
                                ReachedEOF = 1;
                                break;
                        }
                ...
                if (ReachedEOF)
                {
                        ...
                        CrtLocate(cf_rows - 1, 22); putstr("Finished reading file...             ");
