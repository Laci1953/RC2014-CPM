...
/* Array of text lines
   -------------------
*/
char* lp_arr[LINES_MAX];                  /* Text lines pointers array */
...
int main(int argc, char** argv)
{
	...
	CrtClear();
        ...
        {
                /* display help */
                putstr("TE text editor\r\n");
                putstr("Copyright (c) 2015-2021 Miguel Garcia / FloppySoftware\r\n");
                putstr("Adapted for RomWBW by Ladislau Szilagyi\r\n");
                putstr("\r\nUse: >TE [options] [filename]\r\n");
                putstr("\r\nwhere options (lowercase or uppercase) are:\r\n");
                putstr(" -Tn (n = 4 or 8) sets the tabs to n spaces, default is 8\r\n");
                putstr(" -Hnn sets the screen height (up to 99, default is 24)\r\n");
                putstr(" -Wnnn sets the screen width (up to 200, default is 80)\r\n");
		putstr("You can store the options in the TE.CFG file, using the following structure:\r\n");
		putstr("Tn\r\n");
		putstr("Hnn\r\n");
		putstr("Wnnn\r\n");
		putstr("If options are not used on the command line, they will be\r\n");
		putstr(" automatically loaded from the TE.CFG file.\r\n");
...
wrong:
	...
	if (argc < 3)
	{
		// try to open & read TE.CFG file
		// structure:
		//	Tn<CR><LF>
		//	Hnn<CR><LF>
		//	Wnnn<CR><LF>

		if((fp = fopen("TE.CFG", "r")))
		{
			fgets(buf, 10, fp);	//Tn
                        ch = buf[1];
			if (ch == '4' || ch == '8')
        ...
        /* Max. number of lines */
        cf_mx_lines = LINES_MAX;

        /* Max. width of lines */
        ln_max = cf_cols - cf_num - 1;

        InitDynM();     /* init RAM dynamic memory */
        ...
        if(ok)
        {
                ...
                if(!MacroRunning())
                {
                        ...
                        if(box_shc)
                        {
                                ...
                                {
                                        k = i;
                                        XGetString(ln_dat + i, lp_arr[lp_cur]);
...
LoopCopyEx()
{
        ...
        {
                tmp = XStringLen(lp_arr[blk_start + i]);
                ptmp = AllocMem(tmp+1);

                if (ptmp)
                {
                        clp_arr[i] = ptmp;
                        XGetString(tmpbuf, lp_arr[blk_start + i]);
                        XPutString(tmpbuf, clp_arr[i]);
                }
                else
                {
                        FreeClipboard();
...
LoopLeftDel()
{
        ...
        if(ln_dat[0])
        {
                ...
                XGetString(tmpbuf, lp_arr[lp_cur - 1]);
                p = tmpbuf;
