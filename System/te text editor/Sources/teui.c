...
Layout()
{
	...
	/* # of columns */
	CrtLocate(PS_ROW, PS_COL_MAX); putint("%03d", 1 + LINE_SIZE_MAX);
...
/* Refresh editor box
   ------------------
   Starting from box row 'row', line 'line', shifted right with offset.
*/
extern char AutoRepeat;	/* 1 : in auto repeat sequence */
char CrtStatus(void);	/* 0xFF : char ready */
...
int row, line;
{
	...
	for(i = row; i < box_rows; ++i)
	{
		...
		if(line < lp_now) 
		{
			...
			if (strlen(tmpbuf) > offset)
				putstrn(tmpbuf + offset, ln_max);

			/* mark end of line or line truncated , like WS did */
			CrtLocate(BOX_ROW + i, PS_END_LIN);

			if (strlen(tmpbuf + offset) > ln_max)
				eol = '+';
			else
				eol = '<';
...
void MenuFreeMemory(void)
{
	...
	sprintf(tmpbuf, "%u MB available memory", freeMem);
...
#ifdef M64
	CenterText(row++, "Adapted for 64KB RAM RC2014");	
#else
#ifdef M128
#ifdef SC108
	CenterText(row++, "Adapted for RC2014's SC108");	
#endif //SC108
#ifdef MM
	CenterText(row++, "Adapted for RC2014's 32KB ROM + 128KB RAM memory module");
#endif //MM
#ifdef SC114
	CenterText(row++, "Adapted for RC2014's SC114");
#endif //SC114
#ifdef S80
	CenterText(row++, "Adapted for Simple80");
#endif //S80
#endif //M128
#endif //M64
#ifdef M512
	CenterText(row++, "Adapted for RC2014's 512KB RAM module");
#endif
#ifdef Z80ALL
	CenterText(row++, "Adapted for Z80ALL");
#endif
