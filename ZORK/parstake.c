...
#define TakeAllRoutine 0
#define TakeOutOfRoutine 0
--- */
/* code ---------------------------------------------------------------------------- */
...
void ParseActionTake_ext(void)
{
  int num_takes, obj, container, i;
  unsigned short* take = (unsigned short*)AllocBuf; /* unsigned short take[80]; */ /*stores objects to be taken*/

  if (MatchCurWord("all") || MatchCurWord("everything"))
    {
      /* TakeAllRoutine; */
      CallOverlay0((void*)TakeAllRoutine, OVR_DO_PARSE_ACTION_EX2A);
      ReturnOverlay0(0);
    }
  ...
  for (;;)
  {
    ...
    {
      if (CallOverlay1((void*)TakeOutOfRoutine, (short)&container, OVR_DO_PARSE_ACTION_EX2A) /* TakeOutOfRoutine(&container) */) 	    	    ReturnOverlay0(0);
      break;
    }
...
"\xd3\xab\x22\xad\x64\x22\xb0\x65\x74\x77\xf3\xb4\xe3\xf6\x73\x2e"); ReturnOverlay0(0);}
  ...
  for (i=0; i<num_takes; i++)
    if (VerifyTakeableObj(take[i], container, num_takes)) ReturnOverlay0(0);
