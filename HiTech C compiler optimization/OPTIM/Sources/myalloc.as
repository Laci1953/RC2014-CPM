...
;
TRACE	equ	1	;1=print a message when entering a routine
...
;
;<         strings>		<operands       ><shadow code>
;                  ^		^               
;		   |            |		
;        	   TOP_STRINGS	BOTTOM_OPR	
;
DYNM_START	equ	1
DYNM_END	equ	0FF63H
...
BLOCKSIZE	equ	7	;1+6
;
TOP_STRINGS:	defw	DYNM_START	;top strings area (must be < BOTTOM_OPR)
BOTTOM_OPR:	defw	DYNM_END	;bottom operands area (must be > TOP_STRINGS)
...
AllocDynOpr:
	...
	inc	a
	ld	hl,(BOTTOM_OPR)		;compare to bottom of allocated operands area
	cp	h
	jr	z,full			
	or	a			;CARRY=0
	ld	bc,BLOCKSIZE		;size of data
	sbc	hl,bc
	ld	(BOTTOM_OPR),hl		;update bottom of allocated operands area
...
AllocDynStr:
	...
	ld	a,(BOTTOM_OPR+1)	;compare to bottom of allocated operands area
	dec	a
	cp	h
	jr	z,fullstr
	push	hl			
...
_IsValid:
	...
	ld	h,(hl)
	ld	l,a		;HL=addr
	ld	a,(BOTTOM_OPR+1)	;is addr in the allocated area?
	dec	a
	cp	h
