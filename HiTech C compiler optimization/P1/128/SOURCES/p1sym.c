...
----------------------------------- */
    ...
    {
        if (buf && strcmp(/* cp->nVName */ (char*)GetWord((char*)cp+OFF_nVName), buf) == 0) 
...
sym_t *sub_4eed(sym_t *st, uint8_t p2, s8_t *p3, bool v, sym_t *p4) {
			        ...
			        /* st->nMemberList */ (sym_t*)GetWord((char*)st+OFF_nMemberList) == p4))) 
	{
                ...
                if (/* p3->c7 */ c7 == (char)ANODE) 
		{
            ...
            if (var4)
                prError(300, "%s: %s redeclared", /* st->nVName */ GetWord((char*)st+OFF_nVName), var4);
...
/*-----------------------------------------------
        ...
        *ppSym       = nodeAlloc(st->nVName);
        ...
        st           = *ppSym;
------------------------------------------------*/
	pName = (char*)GetWord((char*)st+OFF_nVName);

	index = ComputeHashEntry(pName);

	tmp1 = lookup(pName); 

	tmp2 = nodeAlloc(pName);
...
void sub_516c(register sym_t *st) {
    ...
    {
	p = /* st->nVname */ (char*)GetWord((char*)st+OFF_nVName);

        if (/* st->m18 */ tmp & 1)
            prError(100+37, /* st->nVName */ p);
...
//    }
    ...
    if (s) 
    {
        ...
        PutWord((char*)pn+OFF_nVName, (short)(tmp = (char *)xalloc(strlen(s) + 1)));
        strcpy(/* pn->nVName */ tmp, s);
    } else
        /* pn->nVName = blank; */
	PutWord((char*)pn+OFF_nVName, (short)blank);
        ...
        while (st = var4)
	{
                ...
                if ((st_tmp->m18 & 3) == 2) 
		{
                    switch (var5) 
		    {
                    ...
                    if (var7)
                        prError(300, "undefined %s: %s", var7, st_tmp->nVName);
                        ...
                        if (var5) 
			{
                    ...
                    if (var7)
                        prWarning(300, "unused %s: %s", var7, st_tmp->nVName);
...
sym_t *findMember(sym_t *p1, char *p2) {
    ...
    for (;;) {
        if (p1 == st) {
            prError(300, "%s is not a member of the struct/union %s", p2, /* p1->nVName */ GetWord((char*)p1+OFF_nVName));
            return NULL;
        }
        if (strcmp(/* st->nVName */ (char*)GetWord((char*)st+OFF_nVName), p2) == 0)
            return st;
        /* st = st->nMemberList; */
	st = (sym_t*)GetWord((char*)st+OFF_nMemberList);
    }
