    ...
    if ((tok = yylex()) == (char)T_ID) {
        ...
        if ((tok = yylex()) != (char)T_LBRACE) {
            if (!(/* st->m18 */ GetWord((char*)st+OFF_m18) & 1))
                prError(100+20, /* st->nVName */ GetWord((char*)st+OFF_nVName));
            ungetTok = tok;
        }
...
args_t *sub_65e2(uint16_t p1) {
        ...
        if ((tok = yylex()) == (char)T_3DOT) 
	{
        ...
        if (tok == (char)T_ID && /* yylval.ySym->m20 */ GetByte((char*)(yylval.ySym)+OFF_m20) != (char)T_TYPEDEF)
            if (p1)
                var12 = true;
            else
                prError(17);
        ...
        if (p1 && !sub_5a76(varb, true, T_AUTO)) 
	{
		...
		PutWord((char*)st+OFF_m18, GetWord((char*)st+OFF_m18) | scFlags | 0x20);
                ...
                prError(100+14, /*st->nVName */ GetWord((char*)st+OFF_nVName));
            else 
	    {
                /* vare->nMemberList = st; */
