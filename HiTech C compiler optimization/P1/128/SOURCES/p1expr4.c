...
expr_t *sub_1340(register expr_t *st, expr_t *p2) {
	...
	prError(100+67, (char*)GetWord((char*)var4+OFF_nVName));
    else if ((var2 = findMember(var4, /* p2->t_pSym->nVName */ (char*)GetWord((char*)(p2->t_pSym)+OFF_nVName))))     {
        sub_51cf(var2);
        st       = s13Alloc(T_126);
