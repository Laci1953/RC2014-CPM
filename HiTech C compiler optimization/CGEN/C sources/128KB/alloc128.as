DEBUG		equ	0	;1=trace Alloc128, Free128, GetString, PutString
SC108		equ	1	;1=SC108, 0=32KB ROM + 128KB RAM Phillip Stevens memory module 
ADDRCHECK	equ	0	;1=check upper RAM address
;
...
COND	DEBUG
msgAll:	defm	'Alloc128 size %d'
	defb	0dh,0ah
	defb	0
msgFree:defm	'Free128'
	defb	0dh,0ah
	defb	0
msgGet:	defm	'GetString %s'
	defb	0dh,0ah
	defb	0
msgPut: defm	'PutString %s'
	defb	0dh,0ah
	defb	0
ENDC
;
...
ENDC
	...
	ld	a,l
	or	h
	ret	z		;return NULL if full
	ld	bc,HEADERSIZE	;else add the size of header
	add	hl,bc
	ret
;
;	Free128
;
;void	Free128(void* buf)
;
_Free128:
COND	DEBUG
	ld	hl,msgFree
...
COND	ADDRCHECK
	...
	jr	nz,copy
COND	DEBUG
	push	de		;save dest addr
	call	$ReadString	;if yes, fetch string from upper RAM
	ld	hl,msgGet
	push	hl
	call	_printf
	pop	bc
...
;	This code must be executed at an address >= 8000H
;
StoreShared:
