...
;
;       CTC ports (as in the SC110 SIO/2 + CTCModule)
;
CTC_0   equ     88H
CTC_1   equ     89H
CTC_2   equ     8AH
...
RTC_INT:
	push	af
	push	hl
	ld	hl,counter	;increment 5ms counter
	ld	a,(hl)
	inc	a
	cp	200		;if counter reached 200...
	jr	z,inc_secs
ret_int:
	ld	(hl),a
	pop	hl
	...
	ei
	reti
inc_secs:			;set counter back at 0
	ld	(hl),0
	inc	hl		;...then increment seconds
	ld	a,(hl)
	inc	a
	cp	60		;if seconds reached 60...
	jr	nz,ret_int
	ld	(hl),0		;set seconds back to 0
	inc	hl
	ld	a,(hl)		;...then increment minutes
	inc	a
	cp	60		;if minutes reached 60...
	jr	nz,ret_int
	ld	(hl),0		;set minutes back to 0
	inc	hl
	ld	a,(hl)		;...then increment hours
	inc	a
	cp	24		;if hours reached 24...
	jr	nz,ret_int
	xor	a		;set hours back to 0
	jr	ret_int
;
;	(time) ---> (TimeASCII)
;
