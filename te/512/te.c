...
char  lp_arr_bank; 		/* Text lines pointers array's dynamic memory bank */
char* lp_arr_i_bank; 		/* Pointer of Text lines dynamic memory banks array */
char  lp_arr_i_bank_bank;	/* Text lines dynamic memory banks array's dynamic memory bank */
...
extern char* clp_arr[];		/* Multi-line clipboard lines pointers array */
extern char clp_arr_i_bank[];	/* Multi-line clipboard lines memory banks array */
...
char* FreeArray(char **arr, int count, int flag, char arr_bank, char* arr_i_bank, char arr_i_bank_bank);
...
main(int argc, char** argv)
{
	...
	Init512Banks();	/* init 512KB dynamic memory */
	...
	/* ln_dat = alloc512(&ln_dat_bank, ln_max + 2); */ /* static */

	/* fe_dat = malloc(FORCED_MAX * 2); */
	/* fe_dat = alloc512(&fe_dat_bank, FORCED_MAX * 2); */ /* static */

	/* lp_arr = malloc(cf_mx_lines * 2); */
	/* alloc text lines pointers array */
	lp_arr = (char**)AllocMem(cf_mx_lines * 2, &lp_arr_bank);

	/* alloc text lines dynamic memory banks array */
	lp_arr_i_bank = AllocMem(cf_mx_lines, &lp_arr_i_bank_bank);
	...
	if(lp_arr && lp_arr_i_bank) 
	{
		...
		if((clp_line = AllocMem(ln_max + 1, &clp_line_bank))) 
		{ 
#endif
			i = 0;
#if OPT_BLOCK
		...
		if(!MacroRunning())
		{
			...
			if(box_shc) 
			{
				...
				if(i) 
				{
					...
					setbank(lp_arr_i_bank[lp_cur]);
					strcpy(ln_dat + i, ptmp);
...
LoopCopyEx()
{
	...
	for(i = 0; i < blk_count; ++i) 
	{
		...
		setbank(lp_arr_bank);
		ptmp = lp_arr[blk_start + i];
		setbank(lp_arr_i_bank_bank);
		setbank(lp_arr_i_bank[blk_start + i]);
		...
		if (ptmp)
		{
			...
			if (lp_arr_i_bank[blk_start + i] == clp_arr_i_bank[i])	/* if same bank */
				strcpy(clp_arr[i], ptmp);			/* then do direct copy */
			else
			{							/* else ... */
				strcpy(tmpbuf, ptmp);				/* copy source in tmpbuf... */
				setbank(clp_arr_i_bank[i]);
				strcpy(clp_arr[i], tmpbuf);			/* ...then copy tmpbuf to dest */
			}
		}
		else 
		{
...
LoopLeftDel()
{
	...
	if(ln_dat[0])
	{
		...
		setbank(lp_arr_i_bank_bank);
		setbank(lp_arr_i_bank[lp_cur - 1]);
