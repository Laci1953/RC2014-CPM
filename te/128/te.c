...
****************************************************************************************/
					if (lp_arr_i_bank[lp_cur]==0)
						Xstrcpy(ln_dat + i, lp_arr[lp_cur]);
					else
...
*****************************************************************/
		if (lp_arr_i_bank[blk_start + i]==0)
			tmp = Xstrlen(lp_arr[blk_start + i]);
		else
...
**************************************************************************/
			if (lp_arr_i_bank[blk_start + i]==0)
				Xstrcpy(tmpbuf, lp_arr[blk_start + i]);
			else
				XGetString(tmpbuf, lp_arr[blk_start + i]);

			if (clp_arr_i_bank[i]==0)
				strcpy(clp_arr[i], DropBlanks(tmpbuf));
			else
				PutString(DropBlanks(tmpbuf), clp_arr[i]);
		}
		else 
		{
			FreeClipboard();
...
***************************************************************/
		if (lp_arr_i_bank[lp_cur - 1]==0)
			Xstrcpy(tmpbuf, lp_arr[lp_cur - 1]);
		else
