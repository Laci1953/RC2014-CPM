...
int ReadFile(char* fn)
{
	...
	int i, len;
		...
		RefreshLineCount(i);
                ...
                if(!fgets(ln_dat, 200, fp)) /* ln_max + CR + ZERO */
                        break;
		...
		if ((len=strlen(p)) > ln_max)
		{
			...
			oversize = 1;
                ...
                if (ptmp)
                {
                        PutWord(lp_arr, lp_now, ptmp, bank_lp_arr);
                        PutByte(lp_arr_i_bank, lp_now, bank, bank_lp_arr_i_bank);

                        PutString(p, ptmp, bank);
...
int WriteFile(char* fn)
{
		...
		RefreshLineCount(i);
                ...
                GetString(tmpbuf, GetWord(lp_arr, i, bank_lp_arr), GetByte(lp_arr_i_bank, i, bank_lp_arr_i_bank));
