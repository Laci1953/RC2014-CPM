...
int main(int argc, char** argv)
{
	...
	if (argc == 1)
	{
		...
		printf("Copyright (c) 2015-2021 Miguel Garcia / FloppySoftware\r\n");
#ifdef 	M128
		printf("Adapted for 128KB RAM by Ladislau Szilagyi\r\n");
#else
		printf("Adapted for 512KB RAM by Ladislau Szilagyi\r\n");
#endif	
		printf("\r\nUse: >TE [options] [filename]\r\n");
		printf("\r\nwhere options (lowercase or uppercase) are:\r\n");
		printf(" -Tn (n = 4 or 8) sets the tabs to n spaces, default is 8\r\n");
		printf(" -Hnn sets the screen height (up to 99, default is 24)\r\n");
		printf(" -Wnnn sets the screen width (up to 200, default is 80)\r\n");
		printf("\r\nContinue? (N/n = quit) :");
		...
		while(*++argv) 
		{
			if((argv)[0][0] == '-') 
			{
				...
				case	'H':	//screen height
					...
					if (!cf_rows || cf_rows > 99)
						goto wrong;
					break;
				case	'W':	//screen width
					cf_cols = atoi(argv[0] + 2);
					if (!cf_cols || cf_cols > 200)
						goto wrong;
					break;
				default:
wrong:
	...
	/* For 128KB version - in lower 64KB RAM */
	lp_arr = Alloc(cf_mx_lines * 2, &bank_lp_arr);

	/* alloc text lines dynamic memory banks array */
	/* For 128KB version - in lower 64KB RAM */
	lp_arr_i_bank = Alloc(cf_mx_lines, &bank_lp_arr_i_bank);
	...
	*clp_line = '\0';
	...
	for(i = 0; i < cf_mx_lines; ++i)
		PutWord(lp_arr, i, NULL, bank_lp_arr);
	...
	if(ok) 
	{
		...
		if(!MacroRunning())
		{
			...
			if(box_shc) 
			{
				...
				{
					k = i;
					XGetString(ln_dat + i, GetWord(lp_arr, lp_cur, bank_lp_arr), GetByte(lp_arr_i_bank, lp_cur, bank_lp_arr_i_bank));
...
LoopCopyEx()
{
	...
	{
		tmp = XStringLen(GetWord(lp_arr, blk_start + i, bank_lp_arr), GetByte(lp_arr_i_bank, blk_start + i, bank_lp_arr_i_bank));
		...
		if (ptmp)
		{
			...
			clp_arr[i] = ptmp;
			XGetString(tmpbuf, GetWord(lp_arr, blk_start + i, bank_lp_arr), GetByte(lp_arr_i_bank, blk_start + i, bank_lp_arr_i_bank));
			XPutString(tmpbuf, clp_arr[i], clp_arr_i_bank[i]);
		}
		else 
		{
			FreeClipboard();
...
LoopPaste()
{
	...
	if(clp_count)
	{
		...
		{
			XGetString(tmpbuf, clp_arr[i], clp_arr_i_bank[i]);
...
LoopLeftDel()
{
	...
	if(ln_dat[0])
	{
		...
		XGetString(tmpbuf, GetWord(lp_arr, lp_cur - 1, bank_lp_arr), GetByte(lp_arr_i_bank, lp_cur - 1, bank_lp_arr_i_bank));
		p = tmpbuf;
...
LoopFind()
{
	...
	{
		XGetString(tmpbuf, GetWord(lp_arr, line, bank_lp_arr), GetByte(lp_arr_i_bank, line, bank_lp_arr_i_bank));
		p = tmpbuf;
...
LoopFindNext()
{
	...
	if(find_str[0])
	{
		...
		tmp = XStringLen(GetWord(lp_arr, lp_cur, bank_lp_arr), GetByte(lp_arr_i_bank, lp_cur, bank_lp_arr_i_bank));
