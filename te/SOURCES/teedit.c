...
int BfEdit()
{
        ...
        XGetString(ln_dat, GetWord(lp_arr, lp_cur, bank_lp_arr), GetByte(lp_arr_i_bank, lp_cur, bank_lp_arr_i_bank));
        ...
        while(run)
        {
                ...
                if(upd_col)
                {
                        ...
                        CrtLocate(PS_ROW, PS_COL_CUR); putint("%03d", box_shc + 1);
                }
        ...
        {
                /* Check for changes */
                XGetString(tmpbuf, GetWord(lp_arr, lp_cur, bank_lp_arr), GetByte(lp_arr_i_bank, lp_cur, bank_lp_arr_i_bank));

                if (memcmp(tmpbuf, ln_dat, len))
                {
                        /* Update the changes */
                        XPutString(ln_dat, GetWord(lp_arr, lp_cur, bank_lp_arr), GetByte(lp_arr_i_bank, lp_cur, bank_lp_arr_i_bank));
