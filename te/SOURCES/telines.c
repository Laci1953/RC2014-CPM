...
int line; char *text; int insert;
{
	...
	if ((p = (char*)AllocMem(tmp, &bank)))
	{
		if(insert) 
		{
			...
			{
				PutWord(lp_arr, i, GetWord(lp_arr, i - 1, bank_lp_arr), bank_lp_arr);
				PutByte(lp_arr_i_bank, i, GetByte(lp_arr_i_bank, i - 1, bank_lp_arr_i_bank), bank_lp_arr_i_bank);
			}
		...
		{
			if(GetWord(lp_arr, line, bank_lp_arr))
				Free(GetWord(lp_arr, line, bank_lp_arr), GetByte(lp_arr_i_bank, line, bank_lp_arr_i_bank)); 
		}

		XPutString(text, p, bank);

		PutWord(lp_arr, line, p, bank_lp_arr);
		PutByte(lp_arr_i_bank, line, bank, bank_lp_arr_i_bank);
...
int line, pos;
{
	...
	{
		if (GetByte(lp_arr_i_bank, line, bank_lp_arr_i_bank) == (unsigned char)LOW64)
			strcpy(tmpbuf, ExpandTabs(GetWord(lp_arr, line, bank_lp_arr)) + pos);
		else
		{
			XGetString(tmpbuf1, GetWord(lp_arr, line, bank_lp_arr), GetByte(lp_arr_i_bank, line, bank_lp_arr_i_bank));
...
int line;
{
	...
	PutWord(lp_arr, lp_now, NULL, bank_lp_arr);
...
int line;
{
	...
	p1 = GetWord(lp_arr, line, bank_lp_arr);
	p2 = GetWord(lp_arr, line + 1, bank_lp_arr);

	s1 = StringLen(p1, GetByte(lp_arr_i_bank, line, bank_lp_arr_i_bank));
	s2 = StringLen(p2, GetByte(lp_arr_i_bank, line+1, bank_lp_arr_i_bank));
	...
	if(s1 + s2 <= ln_max) 
	{
		if ((p = (char*)AllocMem(s1+s2+1, &bank)))
		{
			...
			GetString(tmpbuf, p1, GetByte(lp_arr_i_bank, line, bank_lp_arr_i_bank));
			
			/* then strcat(tmpbuf,p2); */
			ptmp = tmpbuf + strlen(tmpbuf);
			GetString(ptmp, p2, GetByte(lp_arr_i_bank, line+1, bank_lp_arr_i_bank));

			/* now, tmpbuf contains strcat(strcpy(p, p1), p2) */
			/* so, we must copy tmpbuf to p */
			PutString(tmpbuf, p, bank);

			PutWord(lp_arr, line, p, bank_lp_arr);
			oldbank = GetByte(lp_arr_i_bank, line, bank_lp_arr_i_bank);
			PutByte(lp_arr_i_bank, line, bank, bank_lp_arr_i_bank);	
